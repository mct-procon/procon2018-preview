...
namespace AngryBee.AI
{
    public class AI_hanpuku_SF_a : MCTProcon29Protocol.AIFramework.AIBase
    {
        ...
        Player Search(int deepness, in (int DestX, int DestY)[] WayEnumerator, in ColoredBoardSmallBigger MeBoard, in ColoredBoardSmallBigger EnemyBoard, in Player Me, in Player Enemy, in sbyte[,] ScoreBoard)
        {
            ...
            while (deepness >= cnt)
            {
                Search_A1(cnt, WayEnumerator, MeBoard, EnemyBoard, Me, Enemy, int.MinValue, int.MaxValue, ScoreBoard);
                BestWay.Agent1 = Me.Agent1 + dp1[cnt].Ag1Way;

                var nMe = Me;
                nMe.Agent1 = BestWay.Agent1;
                var nMeBoard = MeBoard;
                var nEnBoard = EnemyBoard;
                var movable = Checker.MovableCheck(MeBoard, EnemyBoard, nMe, Enemy);
